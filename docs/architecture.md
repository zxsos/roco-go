...
- **心跳空窗里的真实轨迹回放**:输入不变时客户端只发 2.5–3s 一次心跳,而那几秒里玩家可能其实在
  ...
  - 空窗期**照常直线外推**:实测(以补报轨迹为真值)直线外推在「直线巡航」与「静默转弯」两种空窗里
    ...
    任何画法都提前不了。**
  - **外推按速度衰减,不是硬截断**:原先 `Math.min(t, 3.5s)` 在时限内照常匀速外推、到点一刀切零。
    问题在于玩家若**在心跳空窗中途减速停下**,箭头会以全速一路外推过去,再被下一个 stop 包拽回来
    ——这是"停下还往前飘"的主要来源。改为位移 `v·τ·(1-e^(-t/τ))`(τ=0.25s):外推总位移**上限恒为
    v·τ**,不再随时间线性累积。代价是长心跳巡航里箭头不到 1s 就慢下来、暂时落后于真人,但落后可由
    上面的轨迹回放补回,而过冲只能靠"先画错再拽回"补救,后者才是"不跟手"的观感来源。
  ...
  另有一份**点触式走走停停**的抓包(地面 + 骑乘,137 包/80.6s,45 个 stop 包占 33%,骑乘峰值
  29m/s)更能暴露启停问题,已固化为回归度量 `web/scripts/verify-motion.mjs`(fixture 见
  `web/scripts/fixtures/`)。把衰减换上去的实测对比:

  | 指标 | 硬截断(改前) | 速度衰减(改后) |
  | --- | --- | --- |
  | 偏差 均值 / p99 / 最大 | 7.5 / 44.7 / 60.7 m | 7.5 / **16.9** / **20.4** m |
  | 每帧位移 p99 / 最大 | 1.22 / **21.09** m | 1.28 / **2.63** m |
  | >10m 的硬跳帧 | 1 | **0** |

  **仍未解决的固有延迟**:该抓包里客户端发完 stop 后平均沉默约 1.8s(最长 2.9s)才报下一个移动包,
  而这期间玩家往往已经在跑。没有任何包到达 ⇒ 前端无从得知,箭头只能定住等(实测定住时长中位 1.8s),
  等补报轨迹到了再沿真实曲线追平。这是**游戏上报节奏**决定的,改外推公式动不了它;要提前知道只能靠
  后端——但那需要预测,不解决"不知道"。唯一能真正消除"先画错再改回"的是 **lag buffer**:渲染时刻
  固定退后 L(0.2–0.4s),在两已知采样点之间沿真实 `path` 插值,只在末点之后外推。实测它把常见情形
  的抽搐帧从 80 降到 46,但补报轨迹到达时会**瞬间**切到真值(>3m/帧 从 1 帧涨到 40 帧),必须再叠
  一层残余缓动才可用。做它需要后端给 `path` 各点补时间戳(现在只有坐标,前端只能按弧长回放)。
