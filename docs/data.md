...
1. **融合不会改变 `base_skill_id`**。融合只改 `fused_power` 与 `fusion_count`
   ...
   那是错的,已更正。
2. **试炼会给部分技能换 id**:魔能爆的基础 id 是 `7020550`,在试炼里变成
   `7880058`(资料站 788 段整段缺失)。这类**试炼专属 id** 已在 `gen_skills.py` 的
   `EXTRA_SKILL_IDS` 按抓包实证逐个登记,新增时须连同实证依据补进注释。
   判定依据:开局 `0x1951` 里玩家选的 `initial_skill_ids=[7020500, 7020550, 7170210]`,
   而服务器下发的槽位是 `[7020500, 7880058, 7170210]` —— 只有魔能爆被替换;
   且它**开局零融合时就存在**,能耗 0(其效果为「消耗所有能量」,其余技能能耗均为正)。
3. **重名技能填 0**:借用/取念/复写(各 4 个变体)、愿力冲击(15 个精灵专属版)、
   疾风涡轮 —— 按名字无从判断该形态会的是哪一个,填错比不填更糟。
4. **覆盖约 85%**(本项目出现的 227 个形态中 193 个有天生技能资料);无资料时整键缺失。
   站点改版后 `fetch_skill_ids.py` 会抓不到(它靠 HTML class 定位字段),
   id 数为 0 时报错退出而非静默产出错数据。

体积:270 KB(`data/skills.json`)。紧凑数组 + 效果描述共享池(5213 条里只有 364 种
不同效果),比朴素 JSON 省一半以上。
