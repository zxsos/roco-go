...
1. **融合产物查不到名是正常的**。融合会生成新的 `skill_id`(基础技能是
   `702xxxx`/`712xxxx`,融合产物如 `7880058`),那些不在基础技能表里 —— 试炼页
   回退显示原始 id,不是 bug。实测一局 7 个技能里 6 个有名、1 个是融合产物。
2. **重名技能填 0**:借用/取念/复写(各 4 个变体)、愿力冲击(15 个精灵专属版)、
   疾风涡轮 —— 按名字无从判断该形态会的是哪一个,填错比不填更糟。
3. **覆盖约 85%**(本项目出现的 227 个形态中 193 个有天生技能资料);无资料时整键缺失。
4. 站点改版后 `fetch_skill_ids.py` 会抓不到(它靠 HTML class 定位字段),
