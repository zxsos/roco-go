...
**`0x0312` 顶层 `egg_gid[]` 才是「当前在孵蛋器里」的权威列表**:程序收到 `0x0312` 后对账
(store.ReconcileHatching)—— 列表里没有但库中标着在孵的蛋 → 清标记与进度;在列表里的 → 置为
在孵并刷新进度。取出后玩家再开一次孵蛋器,残留标记即被纠正(历史遗留一并清掉)。
`hatched_secs[]` 与 `egg_gid[]` 按下标配对,但 proto3 会省掉 0 值(刚放入的蛋),数组长度
不一致时只对账标记、不刷新进度。

但 `0x0312` 只在打开孵化界面时触发,**重新登录后背包全量(0x1344/登录快照)会把残留
`start_hatch_time` 的蛋重新标成在孵**,等不到 `0x0312` 就被回灌。因此另有两条防线:

- **动作清零**:收到 c2s `0x02ff` 记下要取出的 egg_gid,`0x0300` 回包一到按动作语义
  清这颗蛋的在孵标记(store.ClearHatching,不信字段)。放入后立刻取出也能实时清。
- **时间推断兜底**(pet.Egg.PruneTakenOut):背包全量入库前,若一颗蛋 `start_hatch_time` 残留、
  且 `hatched_secs`/`last_hatch_update_sec` 全为 0、且放入至今已超过孵满时长——放着早该
  孵完却零进度,只能是被取出过 → 清零孵化字段。刚放入(时间差几秒)与正在孵(有进度)的
  蛋不会命中;`0x0300` 动作清零覆盖「刚取出(时间差 < 孵满时长)」的盲区。
