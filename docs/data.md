...
厘米,`z` 为高度,供 3.4 的洞穴层守卫;星点另带 `r:刷新点id` 与 `zone:候选区域`,采集物另带
`i:图标原名`——一个层装 48 个品种,故图标逐点带而非整层共用一个),`poi_kinds` 是
...
(48 个品种),每行给出品种名 `genre` 与图标 `icon`——图标列是 BagItem 编号(100211 可可果),
与不咕钟零件同为「大地图钉直接复用背包图标」,走 `copy_texture`(见 `gen_icons.py` 的
`gather_icons`),故采集物图标就是它产出的那件物品的样子。品种 → NPC id 靠
`MEGAMAP_GATHERING_CONF`:其 `param_id` 即采集物 NPC(50041 向阳花、50090 可可果树…,62 个),
`genre` 与 MEGAMAP_CONF 的品种名逐字相同。点位与星星/零件同走 NPC 白名单直取刷新行
(`refresh_type=1` → `AREA_CONF.center_xyz`),卡洛西亚大陆 **3552 点、41 个品种**
(黑晶琉璃 338、伞伞菌 206、黄石榴石 196…),默认关闭图层;`genre` 为「星芒花」「创愈草」
的两个品种官方没配 `icon`,回退到图层图标。
与星星的根本差别:**它是候选刷新点,不是一次性收集品**——采集物采完会再刷(刷新行全部带
`refresh_rule`),故「走近却没有实体」推不出「已采集」,该图层**不带 `collect` 标记**、不进
收集模式(3.4):图上看的是「哪儿会有」而非「此刻有没有」。同理别拿配置点数当分母——同品种
常有几十上百个候选点,与服务器计数无关。

**实时采集物图层**(2026-09 新增,`GET /api/gathers` + SSE `gathers`):同一批东西的
另一种画法,与上面的候选点图层**并存而非替代**——

- 候选点图层:3552 个配置点位,回答「哪儿会有」,默认关(三千多个点糊屏);
- 实时图层:服务器当下真下发的实体,回答「这会儿有」,默认开(只画那几个)。

判据是**实体的 `npc_content_cfg_id` 命中采集物点位表的 `r`**,不是拿 `npc_cfg_id`
去比 NPC 白名单:一个品种可能有多个 npc_cfg_id(黄石榴石有 50900/50901/50902
三个),而品种名与图标本来就得从点位表取,对上了就一次拿全(见
`gamedata.GatherByRefresh`)。两份 pcap 实测(57s/64 实体、55s/20 实体)全部命中,
品种名与本节的 `genre` 完全对上。
...
| 项 | 值 |
| --- | --- |
| 刷出率 | **三到四成**(87m 圈内 13~19 个候选点里只有 4~6 个有实体) |
| 被采走时玩家距离 | 6~17 m,且**同秒**有 `0x0243` 奖励通知 |
| 出 AOI 时玩家距离 | 63~92 m(AOI 半径约 70~92 m) |

语义与野生宠**相反**:实体离开(采走或出 AOI)即撤标记,**不留「最后所见」灰点**——
采完会再刷,留着等于骗玩家「那儿还有」;再刷时服务器会重发 enter,标记自然回来。
换场景/传送整份作废(传送不为旧实体补发 leave,见 `resetGathers`)。

**别拿 `i`(品种图标的 BagItem 编号)当「采这个给什么」**:实测向阳花(100604)、
伞伞菌(100622)吻合,但海神花 `i`=100755 而实际给的是 100844,且一次采集会给多个物品。

已知缺口(生成脚本侧,不影响实时图层):`GATHER_NPCS` 应有 62 个品种而点位表只落了
41 个,差的那些认不出来。实测 `npc_cfg_id 50047` 在刷实体(点位 r=1001890/1001903),
而点位表里 `1001xxx` 段一条都没有 —— 根因是 `GATHER_GENRES` 要求 `icon` 非空,
把没配图标的品种整体过滤了(「星芒花」「创愈草」同此)。要修的是表,不是实时层的判据。
...
(⚠️ 落库时它会被改写成抓包主机的观测时刻,前端拿到的不是它 —— 见下面「差分」一节)。
2026-08-15 三份 pcap 里相邻两次查询恒为 `Δhatched = 5 × Δ墙钟`(+50/10s、+60/12s),
一次不间断、无道具的孵化满足:

```
hatched_secs = 倍率 × (last_hatch_update_sec − start_hatch_time)   # 截距为 0
```

> **2026-09-05 更正:截距是 0,不是 250。** 这里原先写 `250 + 倍率 × (…)`,
> 是 2026-08-15 那批数据联立解出的。但 2026-09 三份新 pcap、19 个采样点全是**干净
> 的截距 0**:刚入孵的蛋 4475 经过 10s 孵了 50s、4473 经过 4s 孵了 20s —— 严格
> `hatched = 5 × elapsed`。套那个 250 的式子,这 19 个点全部算出**负数**倍率
> (如蛋 4486 经过 1s 孵 5s → `(5−250)/1 = −245`),且各点互不一致,无解。
>
> 反推那个 250 是什么:若真值 r=5 且式子成立,则 `elapsed_real = elapsed + 50`,
> 即**那批记录的 `start_hatch_time` 比真实入孵时刻晚了 50 秒** —— 是观测偏差,
> 不是物理常数(那批蛋是孵了一阵才开界面,`start_hatch_time` 可能取自背包快照;
> 09 这两颗是刚放入、时刻直接来自放入回包 `0x0164`,故无偏差)。写死必然算错。

**外推用的倍率 = 活动倍率(时间表),由后端直接算出**(2026-09-05 定稿)。

这里走过一大段弯路,结论是:**倍率不该测,该按时间表算;而在线加成不该进外推**。
两者性质不同、来源不同,混成一个数去估必然两头不讨好。

| 组成 | 是什么 | 怎么取 |
| --- | --- | --- |
| **活动倍率** | 每周「孵蛋加速日」**北京时间周五 04:00 ~ 周一 04:00**,期间后台按 500% 推进 | **按时刻直接算**(`pet.HatchActivityRate`),窗口内 5、否则 1 |
| **在线加成** | 玩家跑动 / 挂风场 / 孵化宝典,在活动倍率之上**再**加 | 在线时由移动包(0x0133)观测,**只作定性提示,不进 ETA** |

活动倍率是**固定时间表**,服务器在窗口内照此推进,**玩家离线时也一样**。故按时刻算
天然准确:离线外推没问题,不存在「刚打开页面还没校准」的冷启动,也不必攒样本。

在线加成**不进 ETA** 是因为它随速度与移动方式而变,没有可信定值。实测
`PCAPdroid_05_9月_13_26_20`(33 次 0x0312、69 个采样点),把每个差分与该区间内的
移动包关联后规律非常干净:

| 区间 | 倍率 Δv/Δt | 区间内移动包 | speed 均值 | 玩家状态 |
| --- | --- | --- | --- | --- |
| 多数区间 | **5.00** | 0~1 条 | 12~88 | 站着/几乎不动 |
| 601→613 | 25.83 | 39 条 | 714 | 移动中 |
| 617→638 | 16.90 | 135 条 | 904 | 移动中 |

**静止时精确等于活动倍率 5.00,移动时才涨到 16.9~25.8。** 之前把那几个大数当成
「服务器批量补齐的跳变」、想用中位数滤掉 —— 错了,那是玩家真的在跑。混进中位数
得到的数既不是静止倍率也不是移动倍率,离线外推必然错。修正后:ETA 按活动倍率算
(即「站着不动」的保守值,宁可慢不虚报「可破壳」),玩家此刻在移动时额外提示
「移动中,实际更快」。

#### 挂风场**不**加成(2026-09-05 第二份 pcap 实测)

`PCAPdroid_05_9月_15_36_58` 是用户专门抓的对照样本:骑乘走动 / 静止 / 挂风场三种
工况都有(25 次 0x0312、39 个采样点、205 个移动包)。结论是**风场本身不加成**:

| 工况 | 样本 | 倍率 |
| --- | --- | --- |
| 静止(**全程挂着 2 个风场**) | 18 | **5.00,范围 [5.00, 5.00] —— 零方差** |
| 移动(同样挂风场) | 3 段(≥10s) | 22.24 ~ 25.83 |

静止段**全部**挂着风场,倍率仍精确等于活动倍率 —— 若风场有加成,这里必然 >5.00。
两份 pcap 交叉验证一致:上一份的静止段同样全程有风场(第一个风场在采样开始前
4 秒就已挂上),静止倍率也是 5.00。

> 注:玩家反馈「挂风场能加速」,推测是**间接**的 —— 风场让玩家能持续骑乘移动,
> 加成实际来自移动。故不单独识别风场,`hatchMoving` 一个判据已覆盖。

**风场在协议里是能看出来的**(判据记在此,将来若要单独用得上):`0x0414` 的
`aura_info_change`,挂在玩家自己的 `actor_id` 上(`logic_id` 即账号 UID):

```
aura_info_change {
  actor_id: 8131259842557902848        ← 玩家自己
  aura_info {                          ← 挂上
    id: 148298  aura_conf_id: 20002    ← 20002 与 20010 两种
    pos {x,y,z}  radius: 439           ← 半径约 4.4~4.7 米(UE 厘米)
  }
  removed_auras { aura_id: 148298 }    ← 离开
}
```

#### 移动加成尚不能定量

剔除 <10s 的区间后(见下),移动段倍率散布明显:

第三份样本 `PCAPdroid_05_9月_15_47_58` 补上后,合计 6 个移动段(已剔除区间内
**实际静止**的部分,见下):

| pcap | 倍率 | speed 均值 | move_mode |
| --- | --- | --- | --- |
| 13_26_20 | 16.90 | 904 | 地面/飞行混合 |
| 13_26_20 | 25.83 | 714 | 地面/飞行混合 |
| 15_36_58 | 23.75 | 898 | 地面 |
| 15_36_58 | 22.24 | 583 | 地面 |
| 15_36_58 | 25.83 | 371 | 地面 |
| 15_47_58 | **21.68** | 900 | 地面(1/3) |

第四份样本 `PCAPdroid_05_9月_16_07_46`(31 次 0x0312、673 个移动包)是**全程
移动、无静止**的干净样本,6 段 ≥19s 的区间全部有完整移动包覆盖:
...
**倍率与速度无关**(2026-09-05 六份 pcap 定论)。中间走过弯路,值得记下:

- 第一版按「长区间平均速度」算得 r = −0.46,被读成「固定开关量」。该算法有
  缺陷(区间内速度常在剧烈变化,平均速度对应的是混合值),修正后 r 变正。
- 但修正后也**不能据 r 断言相关** —— 严格筛选后 speed≈828 的 3 个样本倍率是
  16.90 / 19.71 / 25.00,**极差 8.10**,比速度从 404 跑到 1726 造成的效应
  (20.62→24.74,跨度 4.12)还大一倍。若倍率由速度决定,同速度样本本该相等。
- 那 8.10 的极差可由**服务器约 5 秒结算一次**的量化误差解释:反解真值区间后
  三者存在重叠 [20.83, 22.18],即它们完全可以是同一真值(约 21.5)。

故**不按速度插值,也不按 `move_mode` 区分**,移动时统一取一个倍率即可。
推论:测量倍率应尽量用**长区间**(dt 越长,步长误差摊得越薄:dt=20s 时 ±1.2,
dt=50s 时 ±0.5)。详见
[pcap-2026-09-05-hatch-rate.md](pcap-2026-09-05-hatch-rate.md) §4.2、§4.3。

四份合计 12 个移动段:中位 **21.96**、均值 22.14、标准差 3.38、范围
[16.90, 27.32];去掉最小最大后中位仍是 21.96、标准差 2.82 —— **分布稳定**。

第五份样本 `PCAPdroid_05_9月_16_16_55`(12 次 0x0312、443 个移动包)是按
「排除过渡态」的思路抓的,给出了**最干净的两段**(时间覆盖 10/10、无空窗):
...
两者只差 1.10,**同一份 pcap 内部的重复性远好于跨文件对比**。

把「覆盖完整(10/10)且停包 <10%」的段挑出来重算,标准差从 3.38 降到 **2.05**:
...
**为什么最终还是折算进了 ETA:实时改变了风险性质**

前几轮拒绝折算,理由是「按中位取会在低端样本上提前 17%(虚报)」。那个推理有个
隐含前提:**倍率要在一段不短的时间里保持不变**,误差才会累积成虚报。

实时显示推翻了这个前提 —— 倍率由两部分拼出,各自都能**当下**拿到:

| 组成 | 怎么拿 | 什么时候变 |
| --- | --- | --- |
| 活动倍率 | 加速日时间表,按当前时刻算 | 只在窗口起止(周五/周一 04:00) |
| 移动增益 | 移动包 0x0133 观测,翻转经 SSE 推送 | 玩家起跑/站住时立刻变 |

于是「误差」不再是「估计不准」,而是「**玩家状态变了**」—— 那是真实发生的事,
不是预测错误。玩家跑动时 ETA 确实短、站住确实长,如实显示是对的。

关键差别:旧方案要**猜**玩家会不会动(故只能取保守下界 18);实时方案是**看**
他此刻动没动,故可取实测中位 **21**(= 活动倍率 × 4.2)而不必退让。

配套上必须做到两点,否则数字乱跳会被当成 bug:
- **移动状态有时效**(10s 无移动包即算停),且前端自己也按时刻过期 —— 玩家站住
  后客户端未必补发 stop 包,不等推送也要能翻回静止;
- **界面标出状态**(徽章「加速中 ×5 · 移动中」、ETA 挂「(移动中)…停下会变慢」),
  让人知道这是「若保持当前状态」的估计。

⚠️ **非加速日 + 移动是推算值**:六份 pcap 全抓在加速日窗口内,没有对照样本区分
「乘法」(1 × 4.2 = 4.2)与「加法」(1 + 16 = 17)两种模型,两者差 4 倍。现按
乘法实现(游戏增益多为乘算),待非加速日样本验证 —— 改动点只有
`pet.hatchMoveGain` 与前端 `MOVE_GAIN` 两个常量。

> 倍率由**前端**按当前时刻算(见 hatch.js 的 hatchRateNow),而非用后端某次
> 响应里的值:挂着页面跨过周一 04:00 加速日就结束了,过期的数字不会自己更新。
> 口径与后端 `pet.HatchRate` 一致,两边改一处要同步改另一处。

**风场数量的影响也一并排除**:三份 pcap 的静止段分别挂着 1 / 2 / 3 个风场,倍率
**都是 5.00** —— 不是「风场少所以看不出」,是真的无关。

> ⚠️ **长区间里可能混着静止,直接算倍率会偏低。** 第三份的 115s 区间倍率只有
> 13.70,看着像「移动加成变小了」;但按 20 秒分段看移动包就会发现**前 55 秒一个
> 移动包都没有**(玩家还没开始跑)。扣掉这段静止(55s × 5 倍)后,剩下 60 秒的
> 真实移动倍率是 21.68 —— 与其余样本一致。统计倍率前务必先确认区间内有没有静止段。

#### 采样粒度:短区间不可信

服务器**约按 5 秒步长**下发进度,故 <10s 的差分区间会给出假值:实测 1s 区间出现
5.00(未跨步长)与 67.50(跨了 3 个步长)两个极端,2s 区间出现 67.50。统计倍率
时必须剔除短区间 —— 上一节表格已只保留 ≥10s 的。

> **已废弃的旧方案**(差分估倍率,留作教训):对相邻两次进度下发做差分、取跨蛋
> 中位数、攒够 3 个样本才给值、持久化到 localStorage。它把活动倍率与在线加成
> 当成同一个量去测,于是样本被移动时段污染、需要冷启动、离线那段完全算不了。
> 而且进度只在开孵蛋器(0x0312)/开背包(0x1344)时下发、**没有被动推送**,
> 前端打开页面时手里通常只有最后一次快照 —— 一次采样压根凑不出差分。
> 这也是最初「16h 的蛋报约剩 8 小时 19 分、真值 1 小时 8 分」的根因。

**倍率的时钟口径照旧**(2026-09 修正)。协议里恰好有两个候选时刻,且会在同一颗蛋上
交替出现:

- 背包快照(0x1344 / 登录)给的是**服务器**的 `last_hatch_update_sec`;
- 0x0312 顶层的 `hatched_secs[]` **不带时刻**,只能配**抓包主机**的消息时刻 `m.Time`。

外推要用「当前值 + 倍率 × (now − HatchUpdate)」,故 `HatchUpdate` 的钟必须与前端的
`now` 同源。故 `store` 落库时一律把它改写成**抓包主机的观测时刻**(见
`internal/store/egg.go` 的 `UpsertEggs`),服务器那个值不采信。前端拿到的
`hatchUpdate` 因此**不是** `last_hatch_update_sec`,读协议字段的人别搞混。

由此还带出一条配套规矩,在同一个文件里:

- **进度为 0 时连时刻一起清零**。否则「旧时刻配 0 进度」会让前端拿 elapsed 去
...
`id 1800022` 那期的窗口是 2026-08-14(周五)04:00:00 ~ 2026-08-17(周一)03:59:59,
文案「背包孵化精灵速度提升至500%」。早几期(1800001~)是「速度增加100%」即 2 倍。
**没有活动时就是 1 倍**,`PET_EGG_CONF.hatch_data` 就是真实秒数(8h / 12h / 16h)。

倍率在配置里只以文案形式存在(`activity_name`/`activity_txt` 里的百分数),
`ACTIVITY_UP_CONF` 那行是空的,没有数值字段可读;且加速活动**不随协议下发**
(0x03c6 的 `activity_data` 里 `activity_type` 只有 22/52/53/58,没有 18)。
故「读配置/协议定倍率」这条路走不通。

但**窗口时间表本身就是稳定的周期规律**,且经两期实测(两份 pcap 都在周六、都测出
5 倍)与玩家确认一致,故直接硬编码在 `pet.HatchActivityRate`(改窗口改那三个常量
即可)。活动 id 每期都变(1800001 → 1800022 → 1800025),**别拿 id 当判据**。

倍率之外还有几个加项,都只在**在线**时发生,叠加在活动倍率之上:

- **移动/挂风场**:实测同一份 pcap 静止时 5.00、移动时 16.9~25.8(见上表)。
- **孵化宝典**:`BAG_ITEM_CONF` 102101/102102/102103/102104,分别 +5%/20%/50%/100%。
...
> **加速日的倍率在玩家离线时也生效**(用户 2026-09-05 实测反馈,与上一条一致):
> 加速日窗口内,玩家下线后服务器**照样按 5 倍推进** `hatched_secs` —— 不是「离线即冻结」。
> 那份跨 1.5 小时「一秒不差符合 5 倍」的 pcap 本就是离线/未操作状态,证据早已在。
> 故前端外推不能假设「离线期间进度不动」:玩家离线回来后刷新,若倍率退回 1 倍,
> offline 那段的推进就被整段低估(见上「倍率采样要持久化」)。
...
但它会误杀**离线超过孵满时长**的真在孵蛋(玩家离线时服务器不重新下发进度,`0x1344`
下发的是放入时的旧快照)——离线一晚、`MaxSec=8h` 的蛋被当成取出过而清掉,故已弃用。

要量化**移动加成**(现在只作定性提示,未进 ETA),可以这样抓一段:开孵蛋器采一次 →
关掉**跑动** 60 秒 → 再开采一次 → **站着不动** 60 秒 → 再开采一次,比两段的
`Δhatched_secs / Δ墙钟`。分工况(地面跑 / 飞行 / 挂风场)各抓一段才能定出各自的数。

进度只在 `0x0312`(开孵蛋器)和 `0x1344`(开背包)里下发,**没有被动推送**,故必须
手动开界面采样 —— 这也正是「倍率不该靠采样」的理由:靠采样的话,玩家不开界面就
永远拿不到新的倍率。

结论:外推用「**当前 `hatched_secs` + 活动倍率 × 已过秒数**」,别从入孵时刻起算;
活动倍率按每周时间表取,不用测。
...
上面这套反推原本只是分析结论,现已实现为 `gamedata.MatchRandomEgg`(2026-09-03)。

数据源在**管理面板**切换(本地源 / 咸鱼源,默认本地),请求参数覆盖不了 ——
数据源是对全服生效的运维选项,若能让请求参数覆盖,任何玩家都能夹带
`src=xianyu` 去烧第三方额度(10 次/分钟)。契约见 `docs/api/schemas.md`。

**本地结果对得上 2026-08-15 那份实测**,而且不止"在候选里"这个程度 —— 重算出的蛋百分位
与当年记录的完全一致:

| | 本节记录的蛋百分位 | 本地实现重算 |
| --- | --- | --- |
| 身高 | 25.0%([19,23] 上的 20) | 25.0% |
| 体重 | 34.1%([9975,14280] 上的 11443) | 34.1% |

两者分毫不差,说明筛选、单位换算、百分位口径三者都对。这条已钉进
`internal/server/api_egg_query_test.go`(改任何一处数字都会漂走,而候选列表照样列得出来)。

咸鱼源**读的是同一张表** —— 它返回的 `hatch_data` / `height_range` /
`weight_range` 正是 `PET_EGG_CONF` 的列。它比本地多的只有系别:系别由协议
`GetSkillDamType()` 逐宠下发,不在任何配置表里,候选物种没抓到过就没有 —— 故本地
候选一律不显示系别。代价是要令牌、限流 10 次/分钟。

**但它不做时长筛选**:只按身高体重匹配,把 `hatch_data` 当展示字段返回。实测同一颗
蛋(0.26m / 2.057kg / 28800s)本地给 4 条、咸鱼源给 12 条,后者 8 条的孵化时长与这颗
蛋不符 —— 其中它还把蹦蹦种子 4 个形态全标成 8h,而解包数据里只有海神球 `3017` 是
28800,其余三个(`3297` 彩玉球 / `3298` 短毛球 / `3299` 象牙球)都是 43200;同时漏了
本该命中的蝴蝶陶陶 `3360001`。故默认本地源不是「退而求其次」,而是更准的那个。

打分则**只能用来排序,不能用来断言「就是它」**:合成测试(两维独立均匀、全表做池)下
真值进前 1 只有 21%、进前 3 只有 51%。这是"尺寸在区间内均匀滚"的必然结果,不是选得不好。
...
`EGG_TYPE_CONF` 里有**没有名字**的品类:实测只有 4 号(`display_order` 102、有角标、
`name` 为空)。它的名字由背包物品反推 —— 该品类下 122 件蛋物品的显示名**全都**是
「活动纪念精灵蛋」,这就是品类名(`gen_gamedata.py` 只在「全部同名」且以「蛋」结尾、
不含「的蛋」时才采信,免得普通蛋那批「喵喵的蛋」被误当品类名)。
品类 8(噩梦)仍无名字:它的蛋全是随机蛋 `conf=0`,没有按物种反查显示名的入口。
...
#### 蛋的外形 `model_id` 与形态组(2026-09-03)

`PET_EGG_CONF` 还有个 `model_id`:**蛋长什么样只由它决定**,与 `conf_id` 是多对一
(917 条蛋只有 295 个 model)。同一物种的血脉变体长得一样(果冻 3118001~005 共 9 个
`pet_id` 共用 model 3118001)。

`conf_id` 前 4 位是**形态组 id**,后 3 位是组内的形态/血脉变体序号。三类条目会把
`model_id` 指向别人:

| 条目 | model 指向 | 例 |
| --- | --- | --- |
| 血脉变体 | 同组基础形态(`001`) | 果冻 3118002(火系血脉)→ 3118001 |
| 自选炫彩蛋(8 位 `3xxx0001`) | 同物种的基础条目 | 喵喵 30010001 → 3001001 |
| 活动纪念蛋(`9900xxxx`) | 它**借用**的那个物种 | 99000055(月牙雪熊)→ 3457001 |

**组号不同的条目之间没有任何推断关系** —— 别拿一个组的 model 去补另一个组的缺。

`model_id` 落在**宠物形态 id 空间**,不保证有对应的蛋配置行。成因是**异色蛋的外形固定取
同组 2 号变体**:110 条异色蛋(`precious=2`)里 109 条的 model 都是 `组号+"002"`
(唯一反例是异色护主犬 3070002,指回 `001`)。于是:

- **地鼠**的两个形态组(`3020` 枯水期 / `3454` 储水时)恰好都**没配 002 号变体**,
  4 条异色蛋的外形因此落空(`3020003`/`3020004`/`3454003`/`3454004`);
- **钨丝贝贝** `3733` 组反过来**缺 001**,2 条蛋的 model 指回不存在的 `3733001`。

共 6 条,是**配置缺失而非解析错误**。结论:取区间/时长**永远用自己的 `conf_id`**,
别拿 model 去查 `egg_conf`(Go 侧见 `gamedata.EggModelConf` 的注释)。

同一图鉴号在蛋表里占多个形态组的,共 **15 个物种**(不止上面那两个):鸭吉吉 6 种
(蓬松/紧实/急急急鸭/等一等鸭/燃了鸭/起来鸭)、雪绒鸟 4 种(春夏秋冬)、丢丢 4 种
(草地/火山/沙地/雪山)、蹦蹦种子 4 种、海枝枝 4 种,以及小星光、梦游、旋叶虫、
小狮鹫、波波螺(图鉴 171,「**噬**波螺」是它的进化态,图鉴 173 —— 注意 `species`
表写「噬」而 `petbase` 写「**嗜**」)、棋棋、刺轮砣、乌达、石肤蜥。
其中石肤蜥的 `3500`(球球尾巴的样子)`image_base` 指向 `3106`(本来的样子),
即两个形态共用一套美术资源。

除上表三类外,还有 2 条 model 跨到了**别的形态组**,性质不同:

| 条目 | model 指向 | 性质 |
| --- | --- | --- |
| `3500002` 石肤蜥(球球尾巴的样子) | `3106001` 石肤蜥(本来的样子) | 同物种跨组,**有意复用** |
| `3062003` 小独角兽(首领血脉) | `3401001` **伊里斯** | **跨物种**,疑为填错 |

前者与它的 `image_base`(形态 `3500` 指向 `3106`)一致,即这两个形态共用一套美术资源,
是有意的;后者是孤例,且它同时是全表唯一带 `close_level`(=5)的条目 —— 像策划手滑,
但 `precious=5`(唯一蛋)全表只有 4 条,样本太少判不了。
...
生成。**注意数据源与其余 gen_*.py 不同** —— 它需要**两份第三方资料**,互补缺一不可:

| 文件 | 来源 | 提供 | 用途 |
| --- | --- | --- | --- |
| `skillIds.json` | `fetch_skill_ids.py` 刮 aismile.dev 技能图鉴(26 页) | **`skill_id` → 中文名**,604 条 | 草系试炼等「只给 id」的场景 |
| `skillGuideData.json` | arkmeng.cn 开放数据 | 威力/能耗/效果 + `innatePets`(哪些精灵天生会、几级学会) | 宠物详情的天生技能表 |

为什么两份都要:

- 只有 id 映射 → 试炼能显示名,但详情页拿不到「几级学会什么」;
- 只有技能详情 → 详情页有数据,但**试炼的裸 id 反查不了**。试炼技能会**融合**,
  融合后威力被改写(实测 `7020500` 融合1次:威力 25→30),按数值反推会命中错误技能
  (30/4 反查得「埋伏」,真身是「乱打」),**只能按 id 精确查**。

**这是过渡方案** —— 权威来源是解包的 `SKILL_CONF`(`base_skill_id` → 中文名,与协议同源)。
等 `scripts/unpack.sh` 解出后应改走解包数据,并删掉 `fetch_skill_ids.py`。

四条必须知道的局限:

1. **融合不会改变 `base_skill_id`**。融合只改 `fused_power` 与 `fusion_count`
   (实测 `7880058` 融合 2 次:威力 20 → 150、fusion_count 0 → 2,id 始终不变),
   故融合态技能**不需要**额外映射 —— 早先以为「融合会生成新 id、查不到是正常的」,
   那是错的,已更正。
2. **试炼会给部分技能换一套编号(`788xxxx`),资料站整段没有这些 id**。
   特征(样本 `PCAPdroid_31_8月_22_54_57`,250 场历史战绩):
   - 至少 27 个 id,分布在 `7880000`~`7880071`,与基础技能(`702`/`704`/`712`…段)不连续;
   - **多数是宠物本来就会的技能**。拿每场战绩的形态去比对账号里那只宠物的实际
     技能列表,已确认的 6 个里有 69%~100% 的场次该形态自己就会这个技能
     (力量增效 100%、霜降 94%、毒孢子 90%、魔能爆 87%、孢子 74%、引燃 69%)。
     故「同一 id 跨形态出现」(如 `7880057` 见于花衣蝶/兽花蕾/帅帅魔偶/影狸)
     **不是**因为与形态无关,而是那些形态恰好都会这个技能 ——
     这点早先写错过一次(写成「试炼侧技能池、与形态无关」),已更正;
   - 剩下 0~26% 的场次形态不会该技能,那些是试炼过程中现给的(节点奖励/技能石),
     这部分按形态推不出来,只能人工在游戏里核对;
   - 池里的技能**都另有基础 id**(如孢子、毒孢子在资料站里都存在),
     试炼态只是换了个编号;魔能爆是实证最完整的一个(见 `gen_skills.py` 注释)。

   已确认 13 个,登记在 `gen_skills.py` 的 `EXTRA_SKILL_IDS`(数值全部与资料站吻合,
   攻击技连威力+能耗都精确对上):

   | 试炼态 id | 技能 | | 试炼态 id | 技能 |
   | --- | --- | --- | --- | --- |
   | `7880000` | 力量增效 | | `7880029` | 虫群智慧 |
   | `7880007` | 热身运动 | | `7880054` | 滚雪球 |
   | `7880008` | 藤绞 | | `7880056` | 暴风雪 |
   | `7880011` | 引燃 | | `7880057` | 孢子 |
   | `7880018` | 霜降 | | `7880058` | 魔能爆 |
   | `7880026` | 毒孢子 | | `7880062` | 光合作用 |
   | | | | `7880068` | 疫病吐息 |

   脚本会校验每个名字都存在于技能表(防手抄错字),对不上直接报错退出。
   新增时请补「怎么确认的」—— 早先给 `7880058` 编了个「融合产物」的解释、没验证
   就写进文档,结果它是魔能爆,这个坑别再踩。
3. **重名技能填 0**:借用/取念/复写(各 4 个变体)、愿力冲击(15 个精灵专属版)、
   疾风涡轮 —— 按名字无从判断该形态会的是哪一个,填错比不填更糟。
   `EXTRA_SKILL_IDS` **只用于查名、不参与天生技能的反查**,否则「孢子」这类会
   因同时有基础 id 与 788 段 id 被误判重名。
4. **覆盖率 98.7%**(历史战绩 1117 个技能实例中 1102 个能查到名;登记试炼池前是 76%),
   剩余 14 个 id 均为低频(各 1~2 次)。形态侧覆盖 462 个形态,无资料时整键缺失。
   站点改版后 `fetch_skill_ids.py` 会抓不到(它靠 HTML class 定位字段),
   id 数为 0 时报错退出而非静默产出错数据。

#### 三类技能与输出格式

宠物详情展示三类,按**获取途径**划分(实测 462 个形态):

| 类别 | 字段 | 含义 | 条目 | 与其他类重叠 |
| --- | --- | --- | --- | --- |
| 天生 | `innate` | 升级自然学会,带学会等级 | 6522 | 与技能石 3 条、与血脉 0 条 |
| 技能石可学 | `stone` | 需消耗技能石才能学 | 7376 | 与天生 3 条 |
| 血脉 | `blood` | 通过血脉获得,**无等级/条件** | 8316 | 与前两类 0 条 |

三者几乎完全互斥,故并列展示、不去重。血脉条目在资料站里只有精灵与进化链,
**没有等级也没有解锁条件**,不要想当然给它补一个等级。

`data/skills.json` 用**技能定义单一来源**格式:

```
skills: [[名, 属性, 威力, 能耗, 效果下标, skill_id], ...]  全局技能表(492 个)
innate: {形态: [[技能下标, 学会等级], ...]}
stone:  {形态: [技能下标, ...]}
blood:  {形态: [技能下标, ...]}
```

22210 条条目其实只涉及 492 个不同技能 —— 定义只存一份、关系存下标,体积
...
640 个候选形态里仍有 71 个(11%)两份资料都查不到。但分类之后
**这 71 个几乎不影响功能** —— 68 个是战斗变体:
...
**剩下 3 个是玩家真能遇到的**,且都能从同图鉴、同进化阶段的普通形态继承:

| 缺的 | 可继承的源 | 特性 |
| --- | --- | --- |
| 幽影树（突变的样子）3777 | 幽影树 3064 | 小偷小摸 |
| V110-C1迪莫 16000004 | 迪莫 3004 | 最好的伙伴 |
| 黑化加尔（黑化的样子（团体战特殊id））19000010 | 黑化加尔（黑化的样子）3233 | 一意孤行 |

**没有实现这层继承** —— 收益只有 3 个形态(覆盖率 89% → 89.5%),而其中
...
试炼里「宠物与它的**天生**特性 id 总是同时出现」(`challenge_data` 的
`initial_feature_ids` 与 `trial_pet_data.base_conf_id` 同包下发),于是:

```
形态全名(PetFullName) --查 pet_feature--> 特性名 --绑--> 那天生特性的 288xxx id
```

对**天生**特性成立;试炼中获得的特性是节点随机给的,与精灵本身无关,拿这条去绑
必然是错的 —— 故只对天生特性用,且池里/宠物上出现多条天生时**都不给名**(宁缺勿错)。

命名口径决定成败:`pet_feature` 的键是 wiki 精灵页名,形态后缀用**全角括号** ——
「鸭吉吉（蓬松的样子）」。我方 `petbase` 的 `n` + `f` 两个字段拼成同一个串才对得上
(`gamedata.PetFullName` / `gamedata.go` 里建 `petNames` 反查表的地方),用半角括号或
下划线会**静默查不到**。实测:wiki 494 个键里有 481 个能与形态全名对上(97%);
有图鉴号的 676 个形态里 502 个查得到特性名(74%,去重后的 640 个候选里 472 个)。

**查不到的 168 个不是"新区形态"** —— 按图鉴号区段看是均匀分布的
(0~99:41、100~199:22、200~299:26、300~399:36、400~499:43),
即 wiki 是**随机漏**了一部分,与精灵新旧无关。
...
问「精灵与特性是不是一一对应」要分方向答,两边结论不同:

| 方向 | 结论 | 实测 |
| --- | --- | --- |
| 精灵 → 特性 | **是**,每个形态恰好一个 | 442 个图鉴条目,无一例外 |
| 特性 → 精灵 | **否**,一个特性名被 1~4 只共用 | 52 个特性只给 1 只、60 个给 2 只、73 个给 3 只、1 个给 4 只 |
| 季节/外观/地区/BOSS 形态 | **多对一** ✓ 特性一致 | 26 只冲突里 **0** 只是这类 |
| ⚠️ 进化链 | **会改特性** | 26 只,特性随进化升级 |

最后一栏是唯一超出直觉的:进化会连特性一起升级 ——
生物碱→高浓生物碱、氧循环→深层氧循环、助燃→爆燃、养分重吸收→养分内循环。
故「一只精灵一个特性」**只在形态级成立,跨进化阶段不成立**。

桥接按**形态全名**查(`FeatureNameOfBase`),本就是形态级,故不受影响;
且只在「恰好一条天生特性」时才绑,多出来的宁可不给名(见 `trialOptionNames`)。

同名形态在配置里有多条记录(棋契陛下 8 条、鸭吉吉国王 6 条、钻石蜗 6 条,
共 16 组 52 个形态),名字与头像都完全一样。候选名单因此**按名字去重**
(676 → 640),且 `PetForms` 与 `PetByName` 必须同口径 ——
两处曾不一致(前者只收有图鉴号的形态、后者收全部),导致
「玩家在候选里选 5025、回填成 3048」。守卫见 `TestPetFormsMatchPetByName`。
...
- **技能名是第三方资料的过渡方案**(见上):`skill_id → 名` 已可用且覆盖良好,
  但权威映射仍需解包 `SKILL_CONF`(届时融合产物、重名变体也能查到);
- **性格** `nature_id` 用 `AUDIO_NATURE_CONF`，个别可能与游戏显示略有偏差。

## 6. 远行商人货单的两个数据源(`internal/server/merchant*.go`)

远行商人每天 8/12/16/20 开四轮、每轮 4 小时。货单来自第三方,当前支持两个**互相独立**
的源,同一时刻只有一个生效(配置存 `merchant_source` 单行表,管理员在管理面板切换):

| | 咸鱼源(`xianyu`,默认) | 好游快爆源(`haoyou`) |
| --- | --- | --- |
| 形态 | 第三方 JSON 接口 | onebiji 公开页面抓取 |
| 令牌 | 需 `-egg-api-key` | **无需令牌** |
| 字段 | 商人名/副标题/商品图/类别/本轮倒计时 | 商品名/价格/限购/**商品图(外链)**/**类别(中文)**,无商人名与倒计时 |

两者滞后实测在**同一量级**(整点后约 1 分钟内),故换源的收益是「不需要第三方令牌」
而不是更快 —— 2026-09-02 起咸鱼源实测返回 401,这条路才有意义。

### 6.1 好游快爆源的页面语义(2026-09-02 08:00 档实测)

页面地址 `https://www.onebiji.com/hykb_tools/comm/lkwgmerchant/preview.php?id=1&immgj=0`
(`immgj` 取 0/1 返回完全一致,是页面自有参数,与出图无关)。
每个商品条目是一个 class 含 `all_show` 的元素,其 `data-time` 是**该档期的结束时间**
(Unix 秒),开始时间 = 结束 − 4h。

条目内部的**商品图与类别**都不在正文里,得从这两处取:

- 商品图:`<div class="gitem">` 内的 `<img src>`,是 biligame patchwiki 的 100px 缩略图
  外链(约 12KB;去掉 `thumb/` 与 `/100px-….png` 后缀可拿原图,约 50KB)。页面展示区
  高 110px,缩略图够用。**注意块内有两枚 `<img>`,价格后面那枚是洛克贝币种图标**
  (`//res.3839pic.com/.../icon.png`),不限定在 `gitem` 内取就会把币种图标当成商品图
  —— 每件商品都显示同一个金币图标,而名字价格全对,极难察觉。
- 类别:只在行内 `onclick="showShopinfo(图, 名, 类别, 描述)"` 的**第三个参数**,且页面
  给的就是中文(「炼金材料」「血脉修改道具」),无需再走 KIND_TEXT 映射。第四个参数是
  商品描述(前端与 `merchantItem` 都没有对应字段,暂不落库)。

服务内置定点抓取(每 30s 一次)跑出的时间线:

| 时刻 | 页面内容 |
| --- | --- |
| 07:58:00 ~ 07:59:31 | 09-01 全天四档(6 件)—— 昨天,已结束 |
| 08:00:01 / 08:00:32 | 解析到 0 件 —— **换天空窗,不是选择器失效** |
| 08:01:02 ~ 08:06:35 | 09-02 08:00~12:00(2 件)—— 当天,已开市 |

结论:

- **换天滞后 = 整点后 32~62 秒**(30s 采样间隔,真实切换落在 08:00:32 与 08:01:02 之间)。
- **「一次给出全天四档」只对已结束的营业日成立**。07:58 给的是**昨天**全天;切到 09-02
  后只给**已开市的那一档**,后三档没有 —— 它**不能预告未来档**。
- **两源商品逐条一致**:咸鱼源 08:05:38 回源拿到「蓝晶碧玺 1000 限购 100」「地系血脉秘药
  160000 限购 3」,与页面上的「1000 限购100」「16w 限购3」对应(`16w` = 160000)。
  这条同时证明该源**没有缺件** —— 每轮确实就 2 件。
- **整点后约 30~60 秒页面解析为 0 件是正常切换过程**,既有「第三方返回空时保留旧货单」
  的保护会兜住,**不要**当成站点改版去告警。

> 曾踩过的坑:订阅邮件里的「整点后 X 分 Y 秒」一度**不是**第三方滞后,而是本服务定时器
> 的相位 —— 服务 03:20:37 启动、tick 落在 `:05/:20/:35/:50`,08:00 后第一个 tick 就是
> 08:05:37,于是「滞后 5 分 38 秒」其实是「我们 08:05:37 才去问」。12:00 那轮的
> 「2 分 16 秒」同理,只是触发者是**有人打开了商人页**(handleMerchant 也会回源),
> 而非定时器 —— 判据是该时刻不落在任何 15 分钟 tick 上。已于 2026-09-02 修掉:轮询改为
> 15 秒一趟,并新增「档期刚开始且还没拿到货单」的 30 秒密集重试(见 merchant.go 的
> merchantPoll / merchantCatchupEvery)。

### 6.4 为什么密集重试必须单独存在(一个反直觉的坑)

两个源都在整点后约 1 分钟才切到新一轮,于是**整点后的第一次回源几乎必然拿到空货单**。
若此时套用追补货用的 10 分钟冷却(merchantRefetch),新货单就要等到整点后 10 分钟才进库
—— **比不轮询还慢**。

故判据里「空货单 + 仍在槽开始后 5 分钟内」按 30 秒重试,一旦拿到货单(empty=false)就
回到 10 分钟冷却去追第三方滞后补上的新货。两者语义不同,不能共用一套节奏:前者是等第三
方**切换**(几十秒量级),后者是等它**补货**(十分钟量级)。

### 6.2 归一化:为什么两个源必须同形

库表 `merchant_slots.data` 存的是第三方原始 JSON,而**有货判定**(`merchantBodyHasItems`)、
**订阅邮件**(`merchantNotify`)、**前端 `unwrap`**、**分栏**四处都按
`code∈{0,200} && data.items` 这一种壳来读。让两种格式并存的话这四处都要分支,漏掉任何
一处都是**静默错乱**(编译能过、页面看着也像那么回事)。故好游快爆侧产出与咸鱼源同形的
envelope,换取下游四处零改动 —— 归一化是这套双源里唯一新增的复杂度。

三处**不归一化就会出错**的细节(均有测试锁住,`merchant_haoyou_test.go`):

- 价格文案 `16w` 必须转成整数 `160000` —— 前端直接渲染 `{it.price} 洛克贝`,不转会在
  页面上显示「16w 洛克贝」。
- 末档(20:00 至次日 00:00)的 `time_label` 必须写 `20:00-24:00` 而非 `20:00-00:00` ——
  后者能过格式校验却匹配不上前后端共用的标准时段键,商品会掉进「其他时段」栏。
- 抓取必须带 User-Agent:空 UA 会被站点 403,而「抓取失败」与「页面没数据」在日志里
  长得一模一样,不查根因会误判成站点改版。

### 6.3 切换数据源

切换会**清空 `merchant_slots` 全部行**再按新源重抓当前轮:两源货单格式不同,留着另一份
会被当成新源的数据显示,页面顶部的来源标注也在说谎。代价是切源当天「昨日回顾」为空,
直到下一个营业日的档被缓存(管理面板卡片里写明了)。`merchant_notified` 不清 —— 那是
「这批商品已通知过谁」的记录、与源无关,清掉会让同一批商品对订阅者再发一遍。

只有咸鱼源需要令牌:原先 `handleMerchant` / `merchantEnsure` 开头的「未配令牌直接返回」
短路已改为按源判定,否则切到好游快爆源照样取不到数据 —— 而那恰恰是换源的主要用途。
