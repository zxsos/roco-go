...
**倍率与速度的关系尚未定论**。按「长区间平均速度」算得 r = −0.46,一度被读成
「与速度无关、是固定开关量」;但该算法有缺陷(区间内速度常在剧烈变化,平均速度
对应的倍率是混合值),改用「区间内速度稳定」的段重算后 r = **+0.694**。
问题是严格筛选后**独立样本只剩 4 个**(孵蛋器里 3 颗蛋会把同一区间重复计 3 次),
且同一速度(828)下的两个样本倍率差 2.80 —— 与速度间的差异同量级。
故现有数据**既不能证实也不能否定**速度相关性,详见
[pcap-2026-09-05-hatch-rate.md](pcap-2026-09-05-hatch-rate.md) §4.1。
在定论之前,按「移动不加区分」处理是安全的(它给出的倍率落在实测区间内)。
