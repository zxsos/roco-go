...
**前端**:图层栏「野生宠物」一组(`WILD_LAYERS` 两个开关 + `MEDAL_FILTERS` 四条奖牌滑块),
选择记忆于 localStorage(键 `.v5`):
...
判的是嗓音原值(≥96/≤-96,即百分位 [98,100]/[0,2] 的边界)。存储键带 `.v5`:这一版把奖牌从
开关换成阈值滑块,沿用旧键会让旧选择错位(开关数组里没有 big/small/high/low 了)。
