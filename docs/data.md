...
**仍未折算进 ETA**,决定因素是误差方向而非标准差大小:

| 取哪个值 | 结果 |
| --- | --- |
| 中位 20.41 | **6 个样本里 3 个会虚报**(低于中位的那些提前) → 不可接受 |
| 下界 18.89 | 全部不虚报,最多保守 31%(推迟) → 方向安全 |

用中位有一半样本会虚报「快好了」;用下界则方向安全,但移动时 ETA 只比静止快
3.8 倍、而实际能到 4.9 倍 —— **提示价值打折**。项目原则是「宁可慢、不虚报可
破壳」,故维持现状:ETA 按活动倍率算,移动时提示「实际更快」。
