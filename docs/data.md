...
**egg 表的 `hatching` 列就是「在孵蛋器里」的权威状态,只由 `0x0312` 对账维护**,
放入/取出动作的回包与背包快照一律不写它(新行初始 0,`UpsertEggs` 的 ON CONFLICT
也不更新该列):

- `0x0312` 对账(`ReconcileHatching`)按 `egg_gid[]` **全量重置**该列:在列表=1,其余=0;
  列表为空时本账号所有在孵标记清零——取出/破壳残留的最终收敛点
- `0x0164` 放入、`0x0300` 取出**也写该列**(见下),方向由 opcode 定。这不是退回「按动作
  置位」那套老做法:老做法错在**照抄回包里的 `start_hatch_time`**,而它在取出时是残留值;
  现在只用 opcode 定方向,不再读那个字段
...
动作回包是服务器对「刚放进去 / 刚取出来」的明确答复,opcode 本身就是权威方向,
故当场写定,不再等 `0x0312`/`hatchGids` **也必须随权威列表更新**:它是登录那一刻记下的旧快照,
此后每开一次背包(`0x1344`)都拿它把 `hatching` 列整体覆盖回去。不更新的话,
新入孵的蛋会在下次开背包时被旧列表打回登录时的状态(实测:`0x0312` 刚标好,开一次背包就没了)。
故 `applyHatchSlots` 记住最新版,`applyEggAction` 也同步改它。
