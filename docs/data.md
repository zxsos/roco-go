...
中位 **22.99**、均值 22.70、标准差 3.33、范围 [16.90, 25.83]。

**仍未折算进 ETA**,理由是误差方向不安全:按中位 23.0 估,遇到实际 16.90 那类
样本会把 11.8 分钟说成 8.7 分钟、**提前 26%** —— 那就是虚报「快好了」,与项目
一贯的「宁可慢、不虚报可破壳」相悖。样本也仍偏少(n=6),且各段都混有起步/减速,
下面的剔除只是估算。继续维持:ETA 按活动倍率算,移动时提示「实际更快」。
