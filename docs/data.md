...
**外推该怎么算倍率 —— 用「差分」而非「累计 ÷ 已孵时长」**(2026-09 实测):

| 口径 | 公式 | 刚入孵 | 玩家跑动过之后 |
| --- | --- | --- | --- |
| 差分(瞬时) | `Δv / Δt` | 5.00 | **5.00** ✓ |
| 单点(历史平均) | `v / (hatchUpdate − startHatch)` | 5.00 | **8.10 ~ 9.03** ✗ |

同为蛋 4475,23:51 刚放入时单点算出 5.00;约 100 秒后(26:10)单点变成 8.10,
而**同一时刻的差分仍是 5.00**。原因:单点算的是入孵以来的历史平均,跑动那段的
加速被平摊进去了;差分算的是采样当时的瞬时速率。预测未来该用后者 —— 玩家此刻
站着,未来大概率还站着,拿历史平均外推会虚报「可破壳」。故 `web/src/pages/eggs/hatch.js`
以差分为唯一主口径,单点**不用**(第一次遇到这颗蛋、从没测过倍率时保守退回 1 倍;
但若别的蛋已测出倍率,可借它的中位数顶上,见下)。

倍率是**全局**的,不逐蛋:26:10 那次采样里,三颗蛋(两颗 8h `max_hatched_secs=28800`、
一颗 16h `57600`)在同样 2 秒内全部 +10s,即统一 5.00。故可跨蛋聚合(取中位数)
来抗单颗蛋的采样抖动。借来的中位数只兜底、**不覆盖**这颗蛋自己测出的值
(宝典是否逐蛋生效尚未证实)。

**倍率主要在后端估,随 `/api/eggs` 的 `hatchRate` 下发**(2026-09-05 实测修正)。
早先只在前端内存里算,而它**通常根本算不出来** —— 这正是用户报的「时间不对」:

| | 手里的采样数 | 结果 |
| --- | --- | --- |
| 前端(打开页面时) | 通常**只有一次**(库里躺着的最后一次快照) | 凑不出差分 → 退回 1 倍 |
| 后端(回放/实时抓包) | **全程每一次下发** | 差分随手可得 → 算出 5.0 |

进度只在开孵蛋器(0x0312)、开背包(0x1344)时下发,**没有被动推送**,故前端页面
拿到的往往就只有最后一次快照。实测回放页的 tooltip 长期挂着「按 1 倍速估算,倍率尚未
测出」,16h 的蛋报「约剩 8 小时 19 分」,而真值(5 倍)是 1 小时 8 分 —— 差 7 倍。

故后端在 `store.UpsertEggs` 里顺手记差分样本(写前从库里取旧值,新值在入参里),
存最近 9 个取中位数落 `hatch_rate` 表,`GET /api/eggs` 一并下发。**取中位数而非
单值**是因为服务器不是平滑推进的:2026-09-05 那份 pcap(`PCAPdroid_05_9月_13_26_20`)
22 个差分里 19 个精确 5.00,却混着 16.9 / 25.8 / 130.0 三个异常(服务器偶尔批量补齐
一大段),中位数把它们滤掉、稳定在 5.0。一批下发的几颗蛋共享同一次服务器结算
(实测三颗同秒各 +10s),故整批先取中位数、只贡献一个样本,免得同一次跳变被放大成好几票。

**攒不够 3 个样本就不给倍率**(返回 0,前端退回保守的 1 倍)。3 是能滤掉**单发**
异常的最小值:`[20, 5, 5]` 的中位数是 5,而只有 2 个时 `[20, 5]` 得 12.5、仍被那
一次跳变主导 —— 照给就会把 ETA 虚报成 2.5 倍快。宁可慢,也不要虚报「可破壳」。
样本是**落盘**的(`hatch_rate` 表),服务重启、玩家重开页面都不用重新攒;只有全新
库/新账号的头几次下发才需要从零攒起,而那是玩家正常游玩时自然产生的(该 pcap 里
相邻两次下发只隔 3 秒),**不需要刻意「打开两次」**。

前端的优先级是**自己的差分 > 后端倍率 > 跨蛋中位数 > 1 倍**:后端那份覆盖「一次采样
都没有」的常见场景;自己的差分仍排第一(它是对这颗蛋最直接的证据,宝典是否逐蛋生效
尚未证实)。前端另把测出的采样持久化到 localStorage(按蛋 gid 建键),刷新页面后恢复 ——
页面持续开着时内存里的采样在,刷新不该丢。权衡:玩家刷新网页却不打开游戏时恢复的是
加速日旧倍率,加速日结束后会短暂高估 ETA;但那是「跨加速日窗口且不重新采样」的窄场景,
一开孵蛋器/背包即被新差分校正,比原来「加速日里必错」更可取。

**差分的两个 `t` 必须同一个钟**(2026-09 修正)。协议里恰好有两个候选,且会在
同一颗蛋上交替出现:

- 背包快照(0x1344 / 登录)给的是**服务器**的 `last_hatch_update_sec`;
- 0x0312 顶层的 `hatched_secs[]` **不带时刻**,只能配**抓包主机**的消息时刻 `m.Time`。

跨源相减就把钟差直接放进分母:网关时钟若与游戏服务器差 60 秒,相隔 10 秒的两次
采样会被算成 70 秒,5 倍速算出 0.7 再被钳到下限 1。故 `store` 落库时一律把
`HatchUpdate` 改写成**抓包主机的观测时刻**(见 `internal/store/egg.go` 的
`UpsertEggs`),服务器那个值不采信 —— `m.Time` 是唯一处处可得的钟。前端拿到的
`hatchUpdate` 因此**不是** `last_hatch_update_sec`,读协议字段的人别搞混。

由此还带出两条配套规矩,都在同一个文件里:

- **进度为 0 时连时刻一起清零**。否则「旧时刻配 0 进度」会让前端拿 elapsed 去
  外推,退化成上面被否掉的单点法(跑动过后会虚报成 8~9 倍)。
- **`v` 必须严格递增才算一次有效采样**。观测时刻每次刷新都变,而进度只在开孵蛋器
  /开背包时才由服务器重算,「新 t 配旧 v」是常态而非异常 —— 若照 `Δv ≥ 0` 判定,
  会算成 0 倍再被钳到下限 1,把 5 倍速生生打成 1 倍。同理,已孵满的蛋(进度被
  `maxSecs` 截断、差分必然偏小)不得进跨蛋中位数。
...
(2026-09 那批的活动 id 已换到 `1800025`,同名「500%孵蛋加速日」;活动 id 会随期数变,
别拿它当判据。另:加速活动**不随协议下发** —— 0x03c6 的 `activity_data` 里
`activity_type` 只有 22/52/53/58,没有 18;全线上下发的活动列表里查不到加速日,
故「读活动表定倍率」这条路走不通,只能采样。)
