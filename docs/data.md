...
以差分为唯一主口径,单点**不用**(刷新后只有一次采样时保守退回 1 倍;但若别的蛋
已测出倍率,可借它的中位数顶上,见下)。

倍率是**全局**的,不逐蛋:26:10 那次采样里,三颗蛋(两颗 8h `max_hatched_secs=28800`、
一颗 16h `57600`)在同样 2 秒内全部 +10s,即统一 5.00。故可跨蛋聚合(取中位数)
来抗单颗蛋的采样抖动 —— 它同时是刷新页面后的兜底:内存里的采样全丢时,单看一颗蛋
只有一次采样、只能退回 1 倍,而那在加速日会把预计时间报成 5 倍远。借来的中位数
只兜底、**不覆盖**这颗蛋自己测出的值(宝典是否逐蛋生效尚未证实)。
