...
**倍率采样要持久化到 localStorage,刷新页面后恢复,而不是退回 1 倍**(2026-09-05
修正)。早先「刷新即回到 1 倍、刻意不持久化」是错的 —— 它正是「刷新前后两种时间」的根:
页面持续开着时内存里的采样在、差分测出 5 倍、ETA 快而准;一刷新内存清空,每颗蛋只剩
一次采样、全退回 1 倍,ETA 报成真实时间的 5 倍远。而且加速日期间玩家**离线**服务器照样
按 5 倍推进 `hatched_secs`(见下),离线回来刷新,用 1 倍外推 offline 那段的进度就全错。
故 `hatch.js` 把测出倍率(known)的采样写进 localStorage(按蛋 gid 建键,gid 是服务器
全局唯一的背包物品 id),刷新时恢复。权衡:玩家刷新网页却不打开游戏时,恢复的是加速日
那时的旧倍率,加速日结束后会短暂高估 ETA;但这是「跨加速日窗口且不重新采样」的窄场景,
玩家一在游戏里打开孵蛋器/背包,新差分就会把它校正回真实倍率 —— 比原来「加速日里刷新
必错」更可取。
