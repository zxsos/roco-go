...
**egg 表的 `hatching` 列就是「在孵蛋器里」的权威状态**,只能由动作/权威消息维护,
背包快照一律不写它(新行初始 0,`UpsertEggs` 的 ON CONFLICT 也不更新该列):

- `0x0164` 放入:changes 里入孵时刻非 0 的蛋 → `SetEggHatcher(gid, true)`
- `0x02ff/0x0300` 取出:收到 c2s 记下 egg_gid,回包一到按动作语义
  `ClearHatching(gid)`(不信字段)。放入后立刻取出也能实时清
- `0x0312` 对账:`ReconcileHatching` 按 `egg_gid[]` 全量重置该列(在列表=1,其余=0),
  历史遗留/异常链路在此收敛
