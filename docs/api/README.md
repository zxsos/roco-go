...
> 好消息是 position / wildpets / home / flowers 四个实时接口的字段已由
...
2. **载荷类型定义** `internal/server/payload.go` —— position / wildpets / home / flowers
   四个实时接口的字段由 Go struct 定义，pipeline 与 server 共用同一份。
3. 本文档
...
> ⚠️ **`position` / `wildpets` / `home` / `flowers` 四个接口的 payload 没有 Go struct 定义**，
> 是管线里内联拼出的 `map[string]any`（见 `internal/pipeline/position.go:219` 等）。
> 它们的字段**只能以 golden 快照为准**，读代码推字段不可靠。
...
>
> 改动实时推送侧（position / wildpets / home / flowers）后，请用
