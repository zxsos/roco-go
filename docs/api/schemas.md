...
它对应「**若保持当前状态**」，由两部分拼出，各自都能当下拿到，故可实时：

| 组成 | 来源 | 何时变 |
| --- | --- | --- |
| 活动倍率 | 加速日时间表，按当前时刻算（`pet.HatchActivityRate`） | 只在窗口起止 |
| 移动增益 | 移动包 0x0133 观测（`hatchMoving`） | 玩家起跑/站住时 |

即 `hatchRate = 活动倍率 × (移动 ? 4.2 : 1)`（见 `pet.HatchRate`）。加速日窗口是
每周**北京时间周五 04:00 ~ 周一 04:00**，服务器在窗口内照此推进、**玩家离线期间
也一样**，故离线外推天然准确，不需要攒样本、不存在冷启动。

`hatchMoving` 报告玩家此刻是否在移动。`hatchMovingAt` 是最近一次观测到在移动的
时刻（从未移动过则为 `time.Time` 零值，是个很大的**负数**）。前端需要它来判断
**时效**：玩家站住后客户端未必补发 stop 包，得自己按时刻过期，否则会一直挂着
「移动中」并按快倍率外推。

> ⚠️ 两处要注意：
> 1. **离线回放时 `hatchMoving` 恒为 false**（历史包内时刻与 `time.Now()` 差几小时，
>    必然超时；语义上也确实无从得知玩家现在在不在动）。
> 2. **非加速日 + 移动的倍率是推算值**——六份 pcap 全抓在加速日窗口内，无法区分
>    「乘法」(1×4.2=4.2) 与「加法」(1+16=17) 模型，现按乘法实现。
>
> 状态翻转时后端会广播 SSE 事件 **`hatch`**，载荷 `{moving, at}`；倍率请前端按
> 当前时刻自己算（加速日窗口起止时它会变，推一个算好的值会过期）。详见
> `docs/data.md` 3.6。
