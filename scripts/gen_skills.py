...
# 试炼专属 id:资料站的技能表**没有**这些 id(788 段整段缺失),但抓包实测它们
# 就是某个技能 —— 试炼给部分技能换了 id,故在此手工登记。
#
# 7880058 = 魔能爆(试炼态)。三条实证(样本 PCAPdroid_31_8月_22_54_57):
...
#   2. 7880058 开局**零融合**时就存在,故它不是融合产物;融合只会改 fused_power
#      (20 -> 150)与 fusion_count(0 -> 2),**不改 base_skill_id**。
#   3. 能耗 0 是魔能爆的独有指纹(其余技能能耗均为正):其效果是「使用时消耗所有
#      能量,消耗越高伤害越高」,试炼需按融合态动态算威力,故单给一个 788 段 id。
#
# 为什么是硬编码:这类映射只能靠抓包实证逐个得出,资料站查不到,数量也极少
# (试炼实测 8 个技能里只有这一个)。新增时请把上面的实证依据一并补进注释,
# 否则后来者无从判断这条映射还可不可信。
EXTRA_SKILL_IDS = {
    7880058: "魔能爆",
}

# 查名用的表含试炼专属 id;但**回填天生技能时只用原始抓取的 id**(见下),
# 否则「魔能爆」会因同时有 7020550 与 7880058 被当成重名、skill_id 填 0
