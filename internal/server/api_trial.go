...
// 精灵池来自静态配置(gamedata/data/trial.json,由 gen_trial_official.py 从客户端
// 官方 GRASS_TRIAL_* 配置生成;第 7 层 NPC 阵容仍为玩家实测),遇见情况来自
...
func (s *Server) handleTrialEncounters(w http.ResponseWriter, r *http.Request) {
	...
	// 官方配置的展示性元数据(数据源/活动期/每章封面与文案):一次解析,取不到就不填 ——
	// 锦上添花的展示,不该因为它让接口失败(见原 trialDataUpdated 的注释)。
	var meta struct {
	...
	if err := json.Unmarshal(gamedata.TrialJSON(), &meta); err == nil {
		out.Updated = meta.Updated
		out.Source = meta.Source
		out.Activity = meta.Activity["10000"] // 当前期的活动说明(进阶期只在赛季切换时出现)
	}
	...
	for ch := uint32(1); ch <= 3; ch++ {
		...
		if cm, ok := meta.Chapters[strconv.FormatUint(uint64(ch), 10)]; ok {
			book.Image, book.Intro = cm.Image, cm.Intro
		}
